...
class Arff:
    ...
    def __init__(
        ...
        elif isinstance(arff, str) or (
            sys.version_info < (3, 0) and isinstance(arff, unicode)
        ):  # load from path
        ...
        if not self.data is None:
            ...
            self.attr_names = (
                [x for x in range(columns)] if not self.attr_names else self.attr_names
            )
            self.attr_types = (
                ["Unknown" for x in range(columns)]
                if not self.attr_types
                else self.attr_types
            )
            self.str_to_enum = (
                [{} for x in range(columns)]
                if not self.str_to_enum
                else self.str_to_enum
            )
            self.enum_to_str = (
                [{} for x in range(columns)]
                if not self.enum_to_str
                else self.enum_to_str
            )
            self.label_columns = []
                ...
                if len(line) > 0 and line[0] != "%":
                    if not reading_data:
                        ...
                        for i, val in enumerate(vals):
                            ...
                            if not val:
                                raise Exception(
                                    "Missing data element in row with data '{}'".format(
                                        line
                                    )
                                )
                            else:
                                if self.numeric:  # record indices for nominal variables
                                    row[val_idx] = float(
                                        self.MISSING
                                        if val == "?"
                                        else self.str_to_enum[val_idx].get(val, val)
                                    )
    ...
    def create_subset_arff(self, row_idx=None, col_idx=None, label_count=None):
        ...
        """
        new_arff = Arff(
            arff=self, row_idx=row_idx, col_idx=col_idx, label_count=label_count
        )  # create a copy
        return new_arff

    def _copy_and_slice_arff(
        self,
        arff=None,
        row_idx=None,
        col_idx=None,
        ...
        def slicer(_list, idx):
            ...
            try:
                ...
                warnings.warn(
                    "Could not slice {} element of Arff object, returning None".format(
                        _list
                    )
                )
                return None
        ...
        if self.is_iterable(row_idx) and self.is_iterable(col_idx):
            warnings.warn(
                "User is attempting to slice both axes using lists. This will result in a 1D array, "
        ...
        if row_idx is None:
            ...
            row_idx = slice(
                row_idx, row_idx + 1
            )  # make it a list, to preserve dimension
        if col_idx is None:
        ...
        if label_count is None and arff.label_count:
            label_list = [
                1 if i in range(column_count - arff.label_count, column_count) else 0
        ...
        if len(self.shape) < 2:
            warnings.warn(
                "Unexpected array dimension (should be 2, not {})".format(
                    len(self.shape)
                )
            )
    ...
    def get_features(self, row_idx=None):
        ...
        if row_idx is None:
            row_idx = slice(0, None)
        end_idx = (
            None if self.label_count == 0 else -self.label_count
        )  # return all if no labels
        return self.create_subset_arff(
            row_idx=row_idx, col_idx=slice(0, end_idx), label_count=0
        )
    ...
    def get_labels(self, row_idx=None):
        ...
        start_idx = (
            self.shape[1] if -self.label_count == 0 else -self.label_count
        )  # return nothing if no labels
        new_arff = self.create_subset_arff(
            row_idx=row_idx,
            col_idx=slice(start_idx, None),
    ...
    def get_nominal_idx(self):
        nominal_idx = [
            i
            for i, feature_type in enumerate(self.attr_types)
